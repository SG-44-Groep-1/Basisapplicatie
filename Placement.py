...
def place_onder10(master1, master2):
    ...
    for row in data:
        if teller < 5 and 5 < row['price'] < 10:
            ...
            for tag in tagknop:
                knoppie = Button(tagframe, text=tag, cursor="hand2", borderwidth=0, highlightthickness=0, bg="#223c4b", activebackground="#67c1f5", fg="#67c1f5")
                knoppie.bind("<Button-1>", lambda event: steampy_tag(event, master1, master2, tag))
                knoppie.grid(row=7, column=column, padx=2, sticky=E)
                column += 1
            teller += 1
...
def steampy_tag(event, master1, master2, button):

    print(button)
    for widget in master1.winfo_children():
        widget.destroy()
    for widget in master2.winfo_children():
        widget.destroy()
    teller = 0
    for row in data:
        if teller < 5 and button in row['steamspy_tags']:
            ...
            Label(spel_frame, text="Naam: " + str(row['name']), font=("Century Gothic", 12, 'bold'), fg='#c7d5e0',
                  bg="#2a475e", wraplength=400).grid(row=0, column=0, pady=2, padx=2, sticky=W)
            ...
            Label(spel_frame, text='Publisher: ' + str(row['publisher']), bg="#2a475e", font=("Arial", 8),
                  fg="#66c0f4").grid(row=6, column=0, pady=4, padx=5, sticky=W)
            tagframe = Frame(spel_frame, bg="#2a475e", highlightthickness=0, borderwidth=0)
            tagframe.grid(row=7, column=0, pady=2, padx=2, sticky=W)
            tagknop = row['steamspy_tags'].split(';')
            column = 1
            Label(tagframe, text='Tags: ', bg="#2a475e", font=("Arial", 8), fg="#66c0f4").grid(row=7, column=0, padx=5,
                                                                                               sticky=E)
            for tag in tagknop:
                knoppie = Button(tagframe, text=tag, cursor="hand2", borderwidth=0, highlightthickness=0, bg="#223c4b",
                                 activebackground="#67c1f5", fg="#67c1f5")
                knoppie.bind("<Button-1>", lambda event: steampy_tag(event, master1, master2, tag))
                knoppie.grid(row=7, column=column, padx=2, sticky=E)
                column += 1
            teller += 1
        elif 5 <= teller < 10 and button in row['steamspy_tags']:
...
def lookup(master, master2, entry):
